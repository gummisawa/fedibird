...
class Account < ApplicationRecord
  ...
  scope :not_excluded_by_account, ->(account) { where.not(id: account.excluded_from_timeline_account_ids).then { |scope| account&.user&.setting_hide_cat ? scope.where.not(id: Account.where("settings ? 'is_cat'").select(:id)) : scope } }
